...
def starter(selected_modules:list):
    ...
    clear()
    ascii()
    print(f"    [{red}>{reset}] Pick Combo File")
    ...
    if not after_accounts:
        ...
        sleep(1)
        return
    print(f"    [{red}>{reset}] Imported {green}{len(before_accounts)}{reset} Accounts")
    if duplicates != 0:
    ...
    if Checker.proxy_type != 'none':
        print(f"    [{red}>{reset}] Pick Proxy File")
        ...
        if not after_proxies:
            ...
            sleep(1)
            return
        print(f"    [{red}>{reset}] Imported {green}{len(before_proxies)}{reset} Proxies")
        if duplicates != 0:
            print(f"    [{red}>{reset}] Removed {green}{duplicates}{reset} Duplicates")
        sleep(0.5)
    ...
    print(f"\n\n    [{red}>{reset}] Finished Checking")
    print(f"    [{red}>{reset}] Saved to \"{save_path}\"")
    input(f"    [{red}>{reset}] Press Enter To Go Back")
