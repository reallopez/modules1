...
def ascii():
    ...
    print(red+"""
         ...
         ____             ____                        
        / __ \___  ____ _/ / /   ____  ____  ___  ____
       / /_/ / _ \/ __ `/ / /   / __ \/ __ \/ _ \/_  /
      / _, _/  __/ /_/ / / /___/ /_/ / /_/ /  __/ / /_
     /_/ |_|\___/\__,_/_/_____/\____/ .___/\___/ /___/
                                   /_/                
        ...
        match saveType:
            ...
            case "good":
                ...
                if 'https://discord.com/api/webhooks/' in Checker.discord_webhook and use_webhook:
                    ...
                    embed.set_author(name="Real Lopez", url="https://calani.mickey-ye.lol", icon_url="https://i.imgur.com/KCmdXsT.png")
...
def cui(modules:int):
    ...
    while Checker.checking:
        with Checker.print_lock:
            ...
            if Checker.stopping:
                print(f"    [{red}Stopping Checker{reset}]")
                return
            
            print(f"""                                            {red}{modules}{reset} Loaded Module{'s' if modules > 1 else ''}
    ...
    [{red}X{reset}] Stop Checking
    [{red}S{reset}] Save Remaining""")
        sleep(5)
...
def title(modules:int):
    ...
    """
    while Checker.checking:
        Checker.title = f"Real Lopez | Good: {numerize(Checker.good)}  ~  Custom: {numerize(Checker.custom)}  ~  Bad: {numerize(Checker.bad)}  ~  Errors: {numerize(Checker.errors)}  ~  CPM: {numerize(get_cpm())}  ~  Remaining: {numerize(len(Checker.remaining)*modules)}"
...
def title_2():
    ...
    """
    while Checker.checking:
        Checker.title = f"Real Lopez | Good: {numerize(Checker.good)} ~ Bad: {numerize(Checker.bad)} ~ CPM: {numerize(get_cpm())} ~ Remaining: {numerize(len(Checker.remaining))}"
        change_title(Checker.title)
        sleep(0.1)
...
def save_lines():
    ...
    if Checker.checking:
        ...
        clear()
        ascii()
        print(f"    [{red}Saving Remaining Lines{reset}]")
    ...
    def start_recording():
        ...
        def save_lines_func():
            ...
            if Checker.checking and is_focused():
                ...
                clear()
                ascii()
                print(f"    [{red}Salvando linhas restantes{reset}]")
                with open(f"Results/{Checker.time}/remaining_lines.txt","w") as file: file.write("\n".join(Checker.remaining))
                sleep(1)
                clear()
                ascii()
