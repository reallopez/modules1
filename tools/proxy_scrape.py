...
def start():
    change_title(f"Real Lopez | Proxy Scraper")
    reset_stats()
    while 1:
        clear()
        ascii()
        print(f"""
    [{red}1{reset}] Pick Links
    [{red}2{reset}] Use Default Links
    ...
    [{red}X{reset}] Back""")
        ...
        match option:
            case "1":
                clear()
                ascii()
                print(f"    [{red}>{reset}] Pick Link File")
                ...
                if not len(original_links):
                    ...
                    sleep(1)
                    return
                print(f"    [{red}>{reset}] Imported {green}{len(original_links)}{reset} Links")
                if duplicates != 0:
                    print(f"    [{red}>{reset}] Removed {green}{duplicates}{reset} Duplicates")
                sleep(0.5)
                scrape(after_links)
            case "2": scrape()
            case "x": return
...
def scrape(links:list=None):
    ...
    print(f"\n\n    [{red}>{reset}] Saving {green}{len(proxies)}{reset} Proxies")
    ...
    print(f"\n\n    [{red}>{reset}] Finished Scraping")
    print(f"    [{red}>{reset}] Saved to \"{save_path}\\Scraped_Proxies.txt\"")
    input(f"    [{red}>{reset}] Pressione enter para sair")
